...
#define PLUGIN_VERSION_AFK_MANAGER_CSGO "1.0.1 edited by Franc1sco franug"
...
#define AFK_THRESHOLD_CSGO 				30.0 //AFK Kontrol İçin Süre 
...
Handle cvar_active_afk_manager_slay_min_csgo;
Handle cvar_active_afk_manager_slay_time_csgo;
...
//Float
float F_active_afk_manager_slay_time_csgo;
...
//Customs
int C_active_afk_manager_slay_min_csgo;
...
{
	name = "phoneix AFK Manager",
	author = "phônêix.js#0001",
	description = "phônêix AFK Manager",
	version = PLUGIN_VERSION_AFK_MANAGER_CSGO,
	url = "discord.gg/bozkurtlar"
}
/***********************************************************/
/*********************** PLUGIN START **********************/
/***********************************************************/
...
{
	AutoExecConfig_SetFile("phoneix", "sourcemod/phoneix");
	
	LoadTranslations("phoneix/phoneix_afk_manager.phrases");
	
	AutoExecConfig_CreateConVar("phoneix_afk_manager_version", PLUGIN_VERSION_AFK_MANAGER_CSGO, "Version", CVARS);
	
	cvar_active_afk_manager_csgo 					= AutoExecConfig_CreateConVar("phoneix_active_afk_manager",  				"1", 					"AFK Kontrolünü Aç veya Kapat", 											DEFAULT_FLAGS, 		true, 0.0, 		true, 1.0);
	cvar_active_afk_manager_csgo_dev				= AutoExecConfig_CreateConVar("phoneix_active_afk_manager_dev", 			"0", 					"AFK Kontrol Developer Aç veya Kapat", 									DEFAULT_FLAGS, 		true, 0.0, 		true, 1.0);
	
	cvar_active_afk_manager_slay_min_csgo			= AutoExecConfig_CreateConVar("phoneix_afk_manager_slay_min", 			"1", 					"Minimum player to slay player", 										DEFAULT_FLAGS);
	cvar_active_afk_manager_slay_time_csgo			= AutoExecConfig_CreateConVar("phoneix_afk_manager_slay_time", 			"60.0", 				"AFK Slayleme Süresi", 													DEFAULT_FLAGS);
	cvar_active_afk_manager_warn_time_csgo			= AutoExecConfig_CreateConVar("phoneix_afk_manager_warn_time", 			"30.0", 				"Uyarı Meajı Gösterme Süresi", 										DEFAULT_FLAGS);
	cvar_active_afk_manager_immune_csgo				= AutoExecConfig_CreateConVar("phoneix_afk_manager_immune", 				"3", 					"AFK Yetkililer: 0 = DISABLED, 1 = Aynısı, 2 = KICK, 3 = SLAY", 	DEFAULT_FLAGS);
	cvar_active_afk_manager_immune_flag_csgo		= AutoExecConfig_CreateConVar("phoneix_afk_manager_immune_flag", 			"z", 					"Etkilenmeyecek Yetkili Bayrağı Boş = Tüm Bayraklar", 								DEFAULT_FLAGS);
	...
	RegAdminCmd("sm_afkcheck",		Command_AfkManagerCsgo, 		ADMFLAG_CHANGEMAP, 		"AFK'ların Olup Olmadıklarını Söyler.");
...
void HookEvents()
{
	...
	HookConVarChange(cvar_active_afk_manager_slay_min_csgo, 	Event_CvarChange);
	HookConVarChange(cvar_active_afk_manager_slay_time_csgo, 	Event_CvarChange);
...
void UpdateState()
{
	...
	C_active_afk_manager_slay_min_csgo 			= GetConVarInt(cvar_active_afk_manager_slay_min_csgo);
	C_active_afk_manager_immune_csgo 			= GetConVarInt(cvar_active_afk_manager_immune_csgo);
	
	F_active_afk_manager_slay_time_csgo 		= GetConVarFloat(cvar_active_afk_manager_slay_time_csgo);
...
public Action Timer_CheckPlayerAfkManagerCsgo(Handle timer, any data)
{
	...
	B_slay_players = (clients >= C_active_afk_manager_slay_min_csgo && F_active_afk_manager_slay_time_csgo > 0.0);
	...
	for(client = 1; client <= MaxClients; client++)
	{
		...
		if (B_slay_players && C_team_num > 1 && ( !C_active_afk_manager_immune_csgo || C_active_afk_manager_immune_csgo == 2 || !CHECK_ADMIN_IMMUNITY(client)))
		{
			F_time_left = F_active_afk_manager_slay_time_csgo - F_client_afk_time_csgo[client];
			if (F_time_left > 0.0)
			{
				if(F_time_left <= F_active_afk_manager_warn_time_csgo)
				{
					CPrintToChat(client, "%t", "Slay_Warning", RoundToFloor(F_time_left));
				}
			}
			else
			{
				...
				CPrintToChatAll("%t", "Slay_Announce", S_client_name);	
		...
		else if (B_slay_players && (!C_active_afk_manager_immune_csgo || C_active_afk_manager_immune_csgo == 3 || !CHECK_ADMIN_IMMUNITY(client)))
		{
			F_time_left = F_active_afk_manager_slay_time_csgo - F_client_afk_time_csgo[client];
			if (F_time_left > 0.0)
			{
			...
			else
			{
				...
				CPrintToChatAll("%t", "Slay_Announce", S_client_name);
				ServerCommand("sm_slay @alive", client, "%t", "Slay_Message");
			}
		}
	}
